...
//			return new ResponseEntity<t_users >(repo.findByUser(user.getUser()).get(), HttpStatus.OK);
//		} else {
...
//	}
	...
	public @ResponseBody String validate(@RequestParam(name = "user")String user,@RequestParam(name = "pass")String pass) {
		Optional<TUser> u = repo.findById(user);
		System.out.println(user);
		System.out.println(pass);
//		return new t_users("hola", "bb");
		if (u.isPresent() && u.get().getPasswd().equals(pass) && u.get().getStatus().equals("ENABLED")) {
			System.out.println(u.get().getPasswd());
			JSONObject o=new JSONObject();
			o.put("rol",u.get().getTRole().getRoleId());
			System.out.println(o.toString());
			return o.toString();
			//TDepartment
//			return u.get().getTEmployee();
		} else {
	...
	public @ResponseBody String insert(@RequestParam(name = "user")String user,@RequestParam(name = "pass")String pass
			,@RequestParam(name = "rol")int rol,@RequestParam(name = "status")String status) {
		TUser u=new TUser();
		u.setUsername(user);
		System.out.println(user);
		u.setPasswd(pass);
